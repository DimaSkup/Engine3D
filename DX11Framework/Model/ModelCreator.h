...
public:
	...
	Model* CreateAndInitModel(ID3D11Device* pDevice, 
		...
		const std::string & fullPathToModelDataFile = "")  // path to model's data file which is used for importing this model   
	{
		// check input params
		assert(pModelInitializer != nullptr);
		...
		try
		{
			...
			pModel->GetModelDataObj()->SetPathToDataFile(fullPathToModelDataFile);
