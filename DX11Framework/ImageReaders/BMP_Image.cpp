...
void BMP_Image::CopyRawDataInto(_Inout_ std::vector<uint8_t> & outData)
{
	try
	{
	...
	UINT & textureHeight)
{
	...
	//(uint32_t)(DirectX::Colors::Cyan)
	std::vector<uint32_t> dataArr(textureWidth * textureHeight, 0);
	...
	D3D11_SUBRESOURCE_DATA initData = { pixelData.data(), pixelData.size() * sizeof(uint32_t), 0 };
	...
	rowPitch = (desc.Width) * sizeof(uint32_t);
