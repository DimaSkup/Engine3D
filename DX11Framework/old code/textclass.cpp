							...
							D3DXMATRIX baseViewMatrix)
{
	...
	// Intialize the font shader object
	...
	result = UpdateSentence(m_sentence1, "Sanya", 500, 500, 
							0.0f, 0.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information
	result = UpdateSentence(m_sentence2, "Verni sotky", 100, 200, 
							1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information
	result = UpdateSentence(m_sentence3, "LET'S DO CODE", 300, 300,
		255.0/255.0, 0.0/255.0, 255.0/255.0, deviceContext);
	if (!result)
	{
		return false;
	}
						...
						D3DXMATRIX orthoMatrix)
{
	...
	result = RenderSentence(deviceContext, m_sentence3,
							worldMatrix, orthoMatrix);
...
bool TextClass::InitializeSentence(SentenceType** sentence, 
									int maxLength, 
									ID3D11Device* device)
{
	VertexType* vertices;
	...
	*sentence = new SentenceType;
	...
	// Release the index array as it is no longer needer
	delete[] indices;
	indices = nullptr;
								...
								ID3D11DeviceContext* deviceContext)
{
	...
	// Use the font class to build the vertex array from the 
	// sentence text and sentence draw location
	m_Font->BuildVertexArray((void*)vertices, text, drawX, drawY);

	// Lock the vertex buffer so it can be written to
	result = deviceContext->Map(sentence->vertexBuffer, 0,
