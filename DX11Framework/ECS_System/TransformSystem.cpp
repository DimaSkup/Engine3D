...
{
	// return a set of all the entities IDs which the transform component has
	std::set<EntityID> enttsIDs;

	for (const auto& it : pTransform_->enttIdToDataIdx_)
		enttsIDs.insert(it.first);
...
void TransformSystem::SetupEnttData(
	const EntityID& enttID,
