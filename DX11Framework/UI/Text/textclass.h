...
public:
	...
	bool Update(ID3D11DeviceContext* pDeviceContext,
		...
		const DirectX::XMFLOAT4 & newColor);
...
private:
	bool BuildSentence(ID3D11Device* pDevice, 
		...
		const DirectX::XMFLOAT4 & color);  // first of all we create an empty sentence (with empty vertices data) and after we update this sentence with text data
