	...
	SerializedDataHeader& header)
{
	...
	for (size_t idx = 0; idx < header.recordsCount; ++idx)
	...
	EntityManager& entityMgr)
{
	...
	const size_t dataBlockMarker = EntityManager::ENTT_MGR_SERIALIZE_DATA_BLOCK_MARKER;
	const size_t dataCount = entityMgr.ids_.size();
